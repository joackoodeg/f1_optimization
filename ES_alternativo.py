...
if __name__ == '__main__':
    ...
    print(f"\nSeleccionando por torneo de {max(3,int(POP_SIZE*0.075))} contrincantes\n")
    ...
    for gen in range(1, NGEN + 1):
        try:
            #Guardar mejor individuo de la generación anterior
            best_prev = tools.selBest(pop, 1)[0]

            # Selección
            parents = tools.selTournament(pop, POP_SIZE, tournsize=max(3,int(POP_SIZE*0.075)))
            ...
            for ind in offspring:
                if random.random() < PMUT:
                    mutation_type = random.choice([
                        toolbox.compound_mutation,
                        toolbox.remove_pit_mutation,
                        toolbox.add_pit_mutation
                    ])
                    mutation_type(ind)
            ...
            pop[:] = offspring
            pop[0] = toolbox.clone(best_prev)  # Elitismo: conservar el mejor individuo
            ...
            #Informar cantidad de individuos invalidos estrategia
            invalid_strategy_count = sum(1 for ind in pop if not validar_estrategia(ind))
            print(f"[Gen {gen}] Individuos con estrategia inválida: {invalid_strategy_count}")
